...
const my3FavoriteTVShows = ['show1', 'show2', 'show3']
// console.log(my3FavoriteTVShows.join(', ').replace(', show3', ' e show3'))
...
const haveThreeItems = my3FavoriteTVShows.length === 3;
// console.log(haveThreeItems);
...
const typeOfIsRandomTVShowIncluded = typeof isRandomTVShowIncluded
const typeSentence = `O tipo de dado que a const "isRandomTVShowIncluded" armazena é: ${typeOfIsRandomTVShowIncluded}.`
...
*/
const thirtyNine = 39
const phrase = `A string que a "typeSentence" armazena tem mais de ${thirtyNine}  
caracteres? ${String(typeSentence.length > thirtyNine)}.`
...
function isAllFalsy(array){
  let isFalsy = false;
  for(let i = 0; i< array.length; i++){
      isFalsy = Boolean(array[i]) == false;
      if(!isFalsy){
        break
      }
    }
...
console.log(evenNumbers.indexOf(8)> 0);
