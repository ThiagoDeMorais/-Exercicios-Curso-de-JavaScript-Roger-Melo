...
for(let i = 0; i< numbers.length; i++){
  ...
  const condition = numbers[i]<= 90 && numbers[i] >= 11
  if(condition){
...
numberOfNumbers = crazyArray.length - (numberOfBooleans + numberOfStrings )
const phrase = `O crazyArray tem ${numberOfBooleans} booleans, ${numberOfNumbers} números e ${numberOfStrings} strings.`
// console.log(phrase);
/*
  06
...
function formatting(array){
  array[array.length -1] = 'lasItem' + array[array.length -1]
  formattedElement = array.join(', ').replace(', lasItem', ' e ')
...
message = `Numeros ímpares: ${formatting(oddNumbers)}. Números pares: ${formatting(evenNumbers)}.`
console.log(message)
